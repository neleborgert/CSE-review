...
#keep rows only if alpha_new and composite_new have values
scale_reliable_two_infos <- scale_reliable_with_info %>%
  filter(!is.na(alpha) & !is.na(composite))
nrow(scale_reliable_two_infos)
#check if same id has multiple studies with ids single_study
#id 56: 4 scales but only 1 study
#id 180: 3 scales and 3 studies
nrow(scale_reliable_two_infos)-3
```
...
#keep rows only if only certain number of columns have values
#first use

##to check for correct sum number of studies
scale_valid_with_one_infos <- scale_valid_with_info[rowSums(!is.na(scale_valid_with_info[, 3:8])) == 1, ]
nrow(scale_valid_with_one_infos)

## Subset df where exactly two out of the six specified columns are not NA
scale_valid_with_two_infos <- scale_valid_with_info[rowSums(!is.na(scale_valid_with_info[, 3:8])) == 2, ]
## check if same id has multiple studies with ids single_study
## id 56: 3 scales out of four but only 1 study
nrow(scale_valid_with_two_infos)-3
## Subset df where exactly three out of the six specified columns are not NA
scale_valid_with_three_infos <- scale_valid_with_info[rowSums(!is.na(scale_valid_with_info[, 3:8])) == 3, ]
#id 180: 3 scales and 3 studies
## id 56: 1 scale out of four but only 1 study
#includes criterion validity study
nrow(scale_valid_with_three_infos)
## Subset df where exactly four out of the six specified columns are not NA
scale_valid_with_four_infos <- scale_valid_with_info[rowSums(!is.na(scale_valid_with_info[, 3:8])) == 4, ]
nrow(scale_valid_with_four_infos)
## Subset df where exactly five out of the six specified columns are not NA
scale_valid_with_five_infos <- scale_valid_with_info[rowSums(!is.na(scale_valid_with_info[, 3:8])) == 5, ]
nrow(scale_valid_with_five_infos)

#recurring external

##prep
external_recurring_scale["validity_content"][external_recurring_scale["validity_content"] == 0] <- NA

##check
rec_scale_valid_with_one_info <- external_recurring_scale[rowSums(!is.na(external_recurring_scale[, c("validity_content","validity_construct_factor_new","validity_construct_discriminant_type","validity_construct_convergent_type","validity_criterion_type","validity_incremental")])) == 1, ] #2

##analyse
rec_scale_valid_with_two_infos <- external_recurring_scale[rowSums(!is.na(external_recurring_scale[, c("validity_content","validity_construct_factor_new","validity_construct_discriminant_type","validity_construct_convergent_type","validity_criterion_type","validity_incremental")])) == 2, ] #2 
rec_scale_valid_with_three_infos <- external_recurring_scale[rowSums(!is.na(external_recurring_scale[, c("validity_content","validity_construct_factor_new","validity_construct_discriminant_type","validity_construct_convergent_type","validity_criterion_type","validity_incremental")])) == 3, ] #4 + 2 from recurring within (codebook1)
rec_scale_valid_with_four_infos <- external_recurring_scale[rowSums(!is.na(external_recurring_scale[, c("validity_content","validity_construct_factor_new","validity_construct_discriminant_type","validity_construct_convergent_type","validity_criterion_type","validity_incremental")])) == 4, ] #0
#five and six out of six = 0 
```

### Scale overview of main quality criteria

Reporting rigor of methodological scale criteria:
- id, authors, scale_number_new
- item_list, facets
- reliability_alpha_new, reliability_composite_new, reliability_testretest, reliability_splithalf
- validity_content, validity_content_experts, validity_construct_factor_new, validity_construct_factor_fitindices, validity_construct_discriminant_type, validity_construct_convergent_type, validity_criterion_type, validity_incremental

TO DO: include definition and possibly pre/pilot testing

#### First use scales

```{r}
# create subset
overview_scales_firstuse <- selfdeveloped_scale[,c("id","authors","scale_number_new",
                                                   "definition","item_list", "facets",
                                                   "reliability_alpha_new", "reliability_composite_new", "reliability_testretest", "reliability_splithalf",
                                                   "validity_content", "validity_content_experts","validity_construct_factor_new","validity_construct_factor_fitindices", "validity_construct_discriminant_type", "validity_construct_convergent_type", "validity_criterion_type", "validity_incremental")] 

# replace 0 with NA in a specific column
overview_scales_firstuse$definition[overview_scales_firstuse$definition == 0] <- NA
overview_scales_firstuse$validity_content[overview_scales_firstuse$validity_content == 0] <- NA
overview_scales_firstuse$validity_content_experts[overview_scales_firstuse$validity_content_experts == 0] <- NA

# replace non-NA values in specific columns with 1
overview_scales_firstuse$definition[!is.na(overview_scales_firstuse$definition)] <- 1
overview_scales_firstuse$item_list[!is.na(overview_scales_firstuse$item_list)] <- 1
overview_scales_firstuse$facets[!is.na(overview_scales_firstuse$facets)] <- 1
overview_scales_firstuse$reliability_alpha_new[!is.na(overview_scales_firstuse$reliability_alpha_new)] <- 1
overview_scales_firstuse$reliability_composite_new[!is.na(overview_scales_firstuse$reliability_composite_new)] <- 1
overview_scales_firstuse$reliability_testretest[!is.na(overview_scales_firstuse$reliability_testretest)] <- 1
overview_scales_firstuse$reliability_splithalf[!is.na(overview_scales_firstuse$reliability_splithalf)] <- 1
overview_scales_firstuse$validity_content[!is.na(overview_scales_firstuse$validity_content)] <- 1
overview_scales_firstuse$validity_content_experts[!is.na(overview_scales_firstuse$validity_content_experts)] <- 1
overview_scales_firstuse$validity_construct_factor_new[!is.na(overview_scales_firstuse$validity_construct_factor_new)] <- 1
overview_scales_firstuse$validity_construct_factor_fitindices[!is.na(overview_scales_firstuse$validity_construct_factor_fitindices)] <- 1
overview_scales_firstuse$validity_construct_discriminant_type[!is.na(overview_scales_firstuse$validity_construct_discriminant_type)] <- 1
overview_scales_firstuse$validity_construct_convergent_type[!is.na(overview_scales_firstuse$validity_construct_convergent_type)] <- 1
overview_scales_firstuse$validity_criterion_type[!is.na(overview_scales_firstuse$validity_criterion_type)] <- 1
overview_scales_firstuse$validity_incremental[!is.na(overview_scales_firstuse$validity_incremental)] <- 1

# Replace all NA values in the data frame with 0
overview_scales_firstuse[,4:18] <- sapply(overview_scales_firstuse[,4:18], as.numeric)
overview_scales_firstuse[is.na(overview_scales_firstuse)] <- 0

# save as excel sheet
#1 = reported
#0 = not reported
write_csv2(overview_scales_firstuse,"../results/Scale overview of main quality criteria - First Use.csv")
```

#### Recurring use scales
```{r}
## recurring (changes = none + codebook 1 manually)

# create subset
overview_scales_recurring <- external_recurring_scale[,c("id","authors","scale_number_new",
                                                   "definition","item_list", "facets",
                                                   "reliability_alpha_new", "reliability_composite_new", "reliability_testretest", "reliability_splithalf",
                                                   "validity_content", "validity_content_experts","validity_construct_factor_new","validity_construct_factor_fitindices", "validity_construct_discriminant_type", "validity_construct_convergent_type", "validity_criterion_type", "validity_incremental")] 

# replace 0 with NA in a specific column
overview_scales_recurring$definition[overview_scales_recurring$definition == 0] <- NA
overview_scales_recurring$validity_content[overview_scales_recurring$validity_content == 0] <- NA
overview_scales_recurring$validity_content_experts[overview_scales_recurring$validity_content_experts == 0] <- NA

# replace non-NA values in specific columns with 1
overview_scales_recurring$definition[!is.na(overview_scales_recurring$definition)] <- 1
overview_scales_recurring$item_list[!is.na(overview_scales_recurring$item_list)] <- 1
overview_scales_recurring$facets[!is.na(overview_scales_recurring$facets)] <- 1
overview_scales_recurring$reliability_alpha_new[!is.na(overview_scales_recurring$reliability_alpha_new)] <- 1
overview_scales_recurring$reliability_composite_new[!is.na(overview_scales_recurring$reliability_composite_new)] <- 1
overview_scales_recurring$reliability_testretest[!is.na(overview_scales_recurring$reliability_testretest)] <- 1
overview_scales_recurring$reliability_splithalf[!is.na(overview_scales_recurring$reliability_splithalf)] <- 1
overview_scales_recurring$validity_content[!is.na(overview_scales_recurring$validity_content)] <- 1
overview_scales_recurring$validity_content_experts[!is.na(overview_scales_recurring$validity_content_experts)] <- 1
overview_scales_recurring$validity_construct_factor_new[!is.na(overview_scales_recurring$validity_construct_factor_new)] <- 1
overview_scales_recurring$validity_construct_factor_fitindices[!is.na(overview_scales_recurring$validity_construct_factor_fitindices)] <- 1
overview_scales_recurring$validity_construct_discriminant_type[!is.na(overview_scales_recurring$validity_construct_discriminant_type)] <- 1
overview_scales_recurring$validity_construct_convergent_type[!is.na(overview_scales_recurring$validity_construct_convergent_type)] <- 1
overview_scales_recurring$validity_criterion_type[!is.na(overview_scales_recurring$validity_criterion_type)] <- 1
overview_scales_recurring$validity_incremental[!is.na(overview_scales_recurring$validity_incremental)] <- 1

# replace all NA values in the data frame with 0
overview_scales_recurring[,4:18] <- sapply(overview_scales_recurring[,4:18], as.numeric)
overview_scales_recurring[is.na(overview_scales_recurring)] <- 0

# add within recurring scales: scale number: 13,74,98,104,71A & scale_origin = 0 ->6 studies/5 scales
id124 <- data.frame(
  id = 124,
  authors = "Cavus et al.",
  scale_number_new = 13,
  ...
  item_list=1,
  facets=1,
  reliability_alpha_new=1,
  reliability_composite_new=0,
  reliability_testretest=0,
  reliability_splithalf=0,
  validity_content=0,
  validity_content_experts=0,
  validity_construct_factor_new=0,
  validity_construct_factor_fitindices=0,
  validity_construct_discriminant_type=0,
  validity_construct_convergent_type=0,
  validity_criterion_type =0,
  validity_incremental=0)
id527 <- data.frame(
  id = 527,
  authors = "Sharma",
  scale_number_new = 104,
  definition = 1,
  item_list=1,
  facets=0,
  reliability_alpha_new=1,
  reliability_composite_new=1,
  reliability_testretest=0,
  reliability_splithalf=0,
  validity_content=1,
  validity_content_experts=0,
  validity_construct_factor_new=0,
  validity_construct_factor_fitindices=0,
  validity_construct_discriminant_type=1,
  validity_construct_convergent_type=1,
  validity_criterion_type =0,
  validity_incremental=0)
id607 <- data.frame(
  id = 607,
  authors = "Thompson et al.",
  scale_number_new = "71A",
  definition = 0,
  item_list=1,
  facets=0,
  reliability_alpha_new=1,
...
id612 <- data.frame(
  id = 612,
  authors = "Torten et al.",
  scale_number_new = 74,
  definition = 0,
  item_list=0,
  facets=0,
  reliability_alpha_new=0,
  reliability_composite_new=0,
  reliability_testretest=0,
  reliability_splithalf=0,
  validity_content=0,
...
overview_scales_recurring <- rbind(overview_scales_recurring, id124)
overview_scales_recurring <- rbind(overview_scales_recurring, id527)
overview_scales_recurring <- rbind(overview_scales_recurring, id607)
overview_scales_recurring <- rbind(overview_scales_recurring, id612)
overview_scales_recurring <- rbind(overview_scales_recurring, id648)
overview_scales_recurring <- rbind(overview_scales_recurring, id649)

# save as excel sheet
#1 = reported
#0 = not reported
write_csv2(overview_scales_recurring,"../results/Scale overview of main quality criteria - Recurring.csv")
```
...
#distribution of factor numbers
table(codebook2$factors)
...
```{r}
#as factor
tech_long_data_clean$technology <- as.factor(tech_long_data_clean$technology)

#data frame
tech_data <- data.frame(unclass(summary(tech_long_data_clean$technology, maxsum = 306))) #306 number of observations in tech_long_data_clean
setDT(tech_data, keep.rownames = TRUE)[] #convert row names into first column

#rename columns
colnames(tech_data)[1] <- "technology_name"
colnames(tech_data)[2] <- "technology_frequency"
tech_data
#check for spelling mistakes

#save as discriminant construct counts
#library("readr")
write_csv2(tech_data,"../results/Counts of All Technologies - Codebook1.csv")
```
...
# 9. Newly created vs modified scales
...
ID 56: overwrite for all same scale_authors
ID 94: all already have the same scale_authors
ID 129: both NA
ID 180: all already have the same scale_authors
ID 372: field notes = NA, transcripts = NA, scale = author

```{r}
#ID 56: overwrite for all same scale_authors
selfdeveloped_scale$scale_authors[selfdeveloped_scale$id == 56] <- "Herath et al. (2009); Peace et al. (2003)"

#exclude papers if scale_authors has an entry
new_scales <- subset(selfdeveloped_scale, is.na(scale_authors))

#exclude papers if scale_authors has no entry
modified_scales <- subset(selfdeveloped_scale, !is.na(scale_authors))
```

## Number of first use measures totals
number of measures (161)
```{r}
nrow(new_scales)
nrow(modified_scales)
```

## Number of studies (156)
```{r}
#check double IDs with codebook 1
## new
#129 x 2 and 2 studies -ok
#372 x2 -ok
nrow(new_scales)

## modified
#56 x 4 but only 1 study -3
#94 x 3 but only 1 study = -2
#180 x 3 and 3 studies -ok
nrow(modified_scales)-5
```

## Reliability 

### Number of studies reporting reliability infos (123)
-total
-two out of four reliability estimates (47)
```{r}
## new scales
new_scale_reliable <- data.frame(new_scales$id,
                          new_scales$scale_number_new,
                          new_scales$reliability_alpha_new,
                          new_scales$reliability_composite_new,
                          new_scales$reliability_testretest,
                          new_scales$reliability_splithalf)
colnames(new_scale_reliable) <- c("id","scale_number","alpha", "composite","testretest","splithalf")
new_scale_reliable_with_info <- new_scale_reliable %>% filter_at(vars(alpha, composite, testretest, splithalf),any_vars(!is.na(.)))
nrow(new_scale_reliable_with_info) #29
#keep rows only if alpha_new and composite_new have values
new_scale_reliable_two_infos <- new_scale_reliable_with_info %>%
  filter(!is.na(alpha) & !is.na(composite))
nrow(new_scale_reliable_two_infos) #8

## modified scales
modified_scale_reliable <- data.frame(modified_scales$id,
                          modified_scales$scale_number_new,
                          modified_scales$reliability_alpha_new,
                          modified_scales$reliability_composite_new,
                          modified_scales$reliability_testretest,
                          modified_scales$reliability_splithalf)
colnames(modified_scale_reliable) <- c("id","scale_number","alpha", "composite","testretest","splithalf")
modified_scale_reliable_with_info <- modified_scale_reliable %>% filter_at(vars(alpha, composite, testretest, splithalf),any_vars(!is.na(.)))
nrow(modified_scale_reliable_with_info)-5 #94
#keep rows only if alpha_new and composite_new have values
modified_scale_reliable_two_infos <- modified_scale_reliable_with_info %>%
  filter(!is.na(alpha) & !is.na(composite))
nrow(modified_scale_reliable_two_infos)-3 #39
#4x56
#2x180
```

number of studies reporting validity information (117)
-content (85)
-factor (11)
-discriminant (96)
-convergent (93)
-criterion (1)
-two out of six validity types (31)
-three out of six validity types (56)
-four out of six validity types (7)
...
### Number of studies reporting alpha and composite
-alpha (102)
-composite (76)
```{r}
#count
...
## first use
non_na_alpha_firstuse <- sum(!is.na(scale_reliable_with_info$alpha))
non_na_alpha_firstuse # 102
non_na_composite_firstuse <- sum(!is.na(scale_reliable_with_info$composite))
non_na_composite_firstuse # 76

##new scales
non_na_alpha_new <- sum(!is.na(new_scale_reliable_with_info$alpha))
non_na_alpha_new # 25
non_na_composite_new <- sum(!is.na(new_scale_reliable_with_info$composite))
non_na_composite_new # 12

##modified scales
non_na_alpha_modified <- sum(!is.na(modified_scale_reliable_with_info$alpha))
non_na_alpha_modified # 77
non_na_composite_modified <- sum(!is.na(modified_scale_reliable_with_info$composite))
non_na_composite_modified # 64
```

### Estimation of alpha and composite
-alpha (.868)
-composite (.897)
```{r}
#use data set scale_reliable_with_info and combine with new_scales, match scale_number with new_scale_number
#match via id

## new scale
codebook1_id_size <- codebook1[,c("id","sample_size_new")] #select only relevant columns
codebook1_id_size_unique <- codebook1_id_size[!duplicated(codebook1_id_size$id), ]# delete multiple ids
new_scale_rel_size<- merge(new_scale_reliable_with_info, codebook1_id_size_unique, by = "id", all.x=T)

## modified scale
modified_scale_rel_size<- merge(modified_scale_reliable_with_info, codebook1_id_size_unique, by = "id", all.x=T)

#manual changes
##new
#id 129 -> 64B change N to 1131
new_scale_rel_size$sample_size_new[new_scale_rel_size$scale_number == "64B"] <- 1131

##modified

#id 56 -> mean alpha 0.96 + 0.964 +0.972 +0.977 because only one N
modified_scale_rel_size$alpha[modified_scale_rel_size$scale_number == "12C"] <- ((0.96 + 0.964 +0.972 +0.977)/4)
modified_scale_rel_size$composite[modified_scale_rel_size$scale_number == "12C"] <- ((0.972+0.960+0.963+0.977)/4)
#12E,12D,12F out
modified_scale_rel_size <- subset(modified_scale_rel_size, scale_number!="12E" & scale_number!="12D" & scale_number!="12F")

#id 94 -> mean alpha 0.84 + 0.84 + 0.85 because only one N
modified_scale_rel_size$alpha[modified_scale_rel_size$scale_number == "61A"] <- ((0.84 + 0.84 + 0.85)/3)
#no composite
#61B,61C out
modified_scale_rel_size <- subset(modified_scale_rel_size, scale_number!="61B" & scale_number!="61C")

#id 180 -> mean alpha 0.821 + 0.8485 because only one N change to (353+329+362)
modified_scale_rel_size$sample_size_new[modified_scale_rel_size$scale_number == "119A"] <- 353
modified_scale_rel_size$sample_size_new[modified_scale_rel_size$scale_number == "119B"] <- 329
modified_scale_rel_size$sample_size_new[modified_scale_rel_size$scale_number == "119C"] <- 362
...
#without NA in data frame
new_scales_alpha_data <- subset(new_scale_rel_size, !is.na(new_scale_rel_size$alpha))
new_scales_alpha_data <- subset(new_scales_alpha_data, !is.na(new_scales_alpha_data$sample_size_new))

#weighted mean
weighted.mean(new_scales_alpha_data$alpha, new_scales_alpha_data$sample_size_new) #0.8728921
#library(jtools)
wtd.sd(new_scales_alpha_data$alpha, new_scales_alpha_data$sample_size_new) #0.0600094

### modified scales

#without NA in data frame
modified_scales_alpha_data <- subset(modified_scale_rel_size, !is.na(modified_scale_rel_size$alpha))
modified_scales_alpha_data <- subset(modified_scales_alpha_data, !is.na(modified_scales_alpha_data$sample_size_new))

#weighted mean
weighted.mean(modified_scales_alpha_data$alpha, modified_scales_alpha_data$sample_size_new) #0.8617814
#library(jtools)
wtd.sd(modified_scales_alpha_data$alpha, modified_scales_alpha_data$sample_size_new) #0.0619277

## weighted composite

### new scales
#without NA in data frame
new_scales_composite_data <- subset(new_scale_rel_size, !is.na(new_scale_rel_size$composite))
new_scales_composite_data <- subset(new_scales_composite_data, !is.na(new_scales_composite_data$sample_size_new))

#weighted mean
weighted.mean(new_scales_composite_data$composite, new_scales_composite_data$sample_size_new) #0.8879505
#library(jtools)
wtd.sd(new_scales_composite_data$composite, new_scales_composite_data$sample_size_new) #0.04771964


### modified scales

#without NA in data frame
modified_scales_composite_data <- subset(modified_scale_rel_size, !is.na(modified_scale_rel_size$composite))
modified_scales_composite_data <- subset(modified_scales_composite_data, !is.na(modified_scales_composite_data$sample_size_new))

#weighted mean
weighted.mean(modified_scales_composite_data$composite, modified_scales_composite_data$sample_size_new) #0.9050664
#library(jtools)
wtd.sd(modified_scales_composite_data$composite, modified_scales_composite_data$sample_size_new) #0.05653042
```

## Validity

### Number of studies reporting validity info
-validity info (117)
-two out of six (31)
-three out of six (56)
-four out of six (7)
...
#create data frame
new_scale_valid <- data.frame(new_scales$id,
                          new_scales$scale_number_new,
                          new_scales$validity_content,
                          new_scales$validity_construct_factor_new,
                          new_scales$validity_construct_discriminant_type,
                          new_scales$validity_construct_convergent_type,
                          new_scales$validity_criterion_type,
                          new_scales$validity_incremental)
colnames(new_scale_valid) <- c("id","scale_number","content", "factor","discriminant","convergent","criterion","incremental")

#change content factor labels: 0 = NA
new_scale_valid["content"][new_scale_valid["content"] == 0] <- NA

#keep rows with at least one value
#library(dplyr)
new_scale_valid_with_info <- new_scale_valid %>% filter_at(vars(content, factor, discriminant, convergent, criterion, incremental),any_vars(!is.na(.)))

#count rows
#check if same id has multiple studies with single_study data frame: no double IDs
nrow(new_scale_valid_with_info) #26
...
##to check for correct sum number of studies
new_scale_valid_with_one_infos <- new_scale_valid_with_info[rowSums(!is.na(new_scale_valid_with_info[, 3:8])) == 1, ]
nrow(new_scale_valid_with_one_infos) #9

## Subset df where exactly two out of the six specified columns are not NA
new_scale_valid_with_two_infos <- new_scale_valid_with_info[rowSums(!is.na(new_scale_valid_with_info[, 3:8])) == 2, ]
nrow(new_scale_valid_with_two_infos) #6

## Subset df where exactly three out of the six specified columns are not NA
new_scale_valid_with_three_infos <- new_scale_valid_with_info[rowSums(!is.na(new_scale_valid_with_info[, 3:8])) == 3, ]
nrow(new_scale_valid_with_three_infos) #9

## Subset df where exactly four out of the six specified columns are not NA
new_scale_valid_with_four_infos <- new_scale_valid_with_info[rowSums(!is.na(new_scale_valid_with_info[, 3:8])) == 4, ]
nrow(new_scale_valid_with_four_infos) #2
...
#create data frame
modified_scale_valid <- data.frame(modified_scales$id,
                          modified_scales$scale_number_new,
                          modified_scales$validity_content,
                          modified_scales$validity_construct_factor_new,
                          modified_scales$validity_construct_discriminant_type,
                          modified_scales$validity_construct_convergent_type,
                          modified_scales$validity_criterion_type,
                          modified_scales$validity_incremental)
colnames(modified_scale_valid) <- c("id","scale_number","content", "factor","discriminant","convergent","criterion","incremental")

#change content factor labels: 0 = NA
modified_scale_valid["content"][modified_scale_valid["content"] == 0] <- NA

#keep rows with at least one value
#library(dplyr)
modified_scale_valid_with_info <- modified_scale_valid %>% filter_at(vars(content, factor, discriminant, convergent, criterion, incremental),any_vars(!is.na(.)))
...
nrow(modified_scale_valid_with_info)-3 #91
...
##to check for correct sum number of studies
modified_scale_valid_with_one_infos <- modified_scale_valid_with_info[rowSums(!is.na(modified_scale_valid_with_info[, 3:8])) == 1, ]
nrow(modified_scale_valid_with_one_infos) #14

## Subset df where exactly two out of the six specified columns are not NA
modified_scale_valid_with_two_infos <- modified_scale_valid_with_info[rowSums(!is.na(modified_scale_valid_with_info[, 3:8])) == 2, ]
## check if same id has multiple studies with ids single_study
## id 56: 3 scales out of four but only 1 study
nrow(modified_scale_valid_with_two_infos)-3 #25

## Subset df where exactly three out of the six specified columns are not NA
modified_scale_valid_with_three_infos <- modified_scale_valid_with_info[rowSums(!is.na(modified_scale_valid_with_info[, 3:8])) == 3, ]
nrow(modified_scale_valid_with_three_infos) #47

## Subset df where exactly four out of the six specified columns are not NA
modified_scale_valid_with_four_infos <- modified_scale_valid_with_info[rowSums(!is.na(modified_scale_valid_with_info[, 3:8])) == 4, ]
nrow(modified_scale_valid_with_four_infos) #5

```

### Number of studies reporting which validity types
-content (86)
-factor (11)
-discriminant (96)
-convergent (93)
...
#create data frame
new_scale_factors <- data.frame(new_scales$validity_content,
                            new_scales$validity_construct_factor_new,
                            new_scales$validity_construct_discriminant_type,
                            new_scales$validity_construct_convergent_type,
                            new_scales$validity_criterion_type,
...
#set as vector
new_scale_factors <- sapply(new_scale_factors, as.vector)

#distribution (incl NA and only percentages)
new_content <- data.frame(prop.table(table(new_scale_factors[,1], exclude = NULL)))
new_factor_type <- data.frame(prop.table(table(new_scale_factors[,2], exclude = NULL)))
new_discr_type <- data.frame(prop.table(table(new_scale_factors[,3], exclude = NULL)))
new_conv_type <- data.frame(prop.table(table(new_scale_factors[,4], exclude = NULL)))
new_criterion_type <- data.frame(prop.table(table(new_scale_factors[,5], exclude = NULL)))
new_incremental <- data.frame(prop.table(table(new_scale_factors[,6], exclude = NULL)))

#get totals of distribution
table(new_scale_factors[,1], exclude = NULL) #18
table(new_scale_factors[,2], exclude = NULL) #3
table(new_scale_factors[,3], exclude = NULL) #17
table(new_scale_factors[,4], exclude = NULL) #18
table(new_scale_factors[,5], exclude = NULL) #0
table(new_scale_factors[,6], exclude = NULL) #0

## modified

#create data frame
modified_scale_factors <- data.frame(modified_scales$validity_content,
                            modified_scales$validity_construct_factor_new,
                            modified_scales$validity_construct_discriminant_type,
                            modified_scales$validity_construct_convergent_type,
                            modified_scales$validity_criterion_type,
...
#set as vector
modified_scale_factors <- sapply(modified_scale_factors, as.vector)

#distribution (incl NA and only percentages)
modified_content <- data.frame(prop.table(table(modified_scale_factors[,1], exclude = NULL)))
modified_factor_type <- data.frame(prop.table(table(modified_scale_factors[,2], exclude = NULL)))
modified_discr_type <- data.frame(prop.table(table(modified_scale_factors[,3], exclude = NULL)))
modified_conv_type <- data.frame(prop.table(table(modified_scale_factors[,4], exclude = NULL)))
modified_criterion_type <- data.frame(prop.table(table(modified_scale_factors[,5], exclude = NULL)))
modified_incremental <- data.frame(prop.table(table(modified_scale_factors[,6], exclude = NULL)))

#get totals of distribution
table(modified_scale_factors[,1], exclude = NULL) #68
table(modified_scale_factors[,2], exclude = NULL) #8
table(modified_scale_factors[,3], exclude = NULL) #79
table(modified_scale_factors[,4], exclude = NULL) #75
table(modified_scale_factors[,5], exclude = NULL) #1
table(modified_scale_factors[,6], exclude = NULL) #0
```


### Number of total validity constructs 
[1] 13 discriminant only
[1] 19 convergent only
[1] 298 doublets

#### Validity construct (discriminant): new
```{r}
#as factor
new_scales_discriminant_long_data_clean$discriminant_description <- as.factor(new_scales_discriminant_long_data_clean$discriminant_description)

#data frame
new_scales_discriminant_data <- data.frame(unclass(summary(new_scales_discriminant_long_data_clean$discriminant_description, maxsum = 579))) #579 number of observations in discriminant_long_data_clean
setDT(new_scales_discriminant_data, keep.rownames = TRUE)[] #convert row names into first column

#rename columns
colnames(new_scales_discriminant_data)[1] <- "construct_name"
colnames(new_scales_discriminant_data)[2] <- "construct_frequency"
new_scales_discriminant_data
#check for spelling mistakes

#save as discriminant construct counts
#library("readr")
write_csv2(new_scales_discriminant_data,"../results/Counts of All Discriminant Constructs - Only Selfdeveloped Scales: Newly Created Scales.csv")
```

#### Validity construct (discriminant): modified
```{r}
#as factor
modified_scales_discriminant_long_data_clean$discriminant_description <- as.factor(modified_scales_discriminant_long_data_clean$discriminant_description)

#data frame
modified_scales_discriminant_data <- data.frame(unclass(summary(modified_scales_discriminant_long_data_clean$discriminant_description, maxsum = 579))) #579 number of observations in discriminant_long_data_clean
setDT(modified_scales_discriminant_data, keep.rownames = TRUE)[] #convert row names into first column

#rename columns
colnames(modified_scales_discriminant_data)[1] <- "construct_name"
colnames(modified_scales_discriminant_data)[2] <- "construct_frequency"
modified_scales_discriminant_data
#check for spelling mistakes

#save as discriminant construct counts
#library("readr")
write_csv2(modified_scales_discriminant_data,"../results/Counts of All Discriminant Constructs - Only Selfdeveloped Scales: Modified Scales.csv")
```

#### Validity construct (convergent): new
```{r}
#as factor
new_scales_convergent_long_data_clean$convergent_description <- as.factor(new_scales_convergent_long_data_clean$convergent_description)
new_scales_convergent_data <- data.frame(unclass(summary(new_scales_convergent_long_data_clean$convergent_description, maxsum = 577))) #577 number of observations in convergent_long_data_clean
setDT(new_scales_convergent_data, keep.rownames = TRUE)[] #convert row names into first column

#rename columns
colnames(new_scales_convergent_data)[1] <- "construct_name"
colnames(new_scales_convergent_data)[2] <- "construct_frequency"
new_scales_convergent_data
#check for spelling mistakes

#save as convergent construct counts
#library("readr")
write_csv2(new_scales_convergent_data,"../results/Counts of All Convergent Constructs - Only Selfdeveloped Scales: Newly Created Scales.csv")
```

#### Validity construct (convergent): modified
```{r}
#as factor
modified_scales_convergent_long_data_clean$convergent_description <- as.factor(modified_scales_convergent_long_data_clean$convergent_description)
modified_scales_convergent_data <- data.frame(unclass(summary(modified_scales_convergent_long_data_clean$convergent_description, maxsum = 577))) #577 number of observations in convergent_long_data_clean
setDT(modified_scales_convergent_data, keep.rownames = TRUE)[] #convert row names into first column

#rename columns
colnames(modified_scales_convergent_data)[1] <- "construct_name"
colnames(modified_scales_convergent_data)[2] <- "construct_frequency"
modified_scales_convergent_data
#check for spelling mistakes

#save as convergent construct counts
#library("readr")
write_csv2(modified_scales_convergent_data,"../results/Counts of All Convergent Constructs - Only Selfdeveloped Scales: Modified Scales.csv")
```

#### Find doublets (with frequency): new
```{r}
#as an index to discriminant_data$construct_name
#doublets
dou_split_new <- data.frame(new_scales_discriminant_data$construct_name[new_scales_discriminant_data$construct_name %in% new_scales_convergent_data$construct_name]) #save returned values in dou
as.vector(dou_split_new)
copy_new_scales_discriminant_data <- new_scales_discriminant_data
copy_new_scales_convergent_data <- new_scales_convergent_data
new_scales_d_freq1 <- setDT(copy_new_scales_discriminant_data, key = 'construct_name')[dou_split_new]
new_scales_d_freq2 <- setDT(copy_new_scales_convergent_data, key = 'construct_name')[dou_split_new]
new_scales_doublets_data <- data.frame(new_scales_d_freq1[, 1], new_scales_d_freq1[, 2] + new_scales_d_freq2[, 2]) #make new data frame: take freq1 country_name and sum frq1 + freq1 construct_frequency
new_scales_doublets_data

#only discriminant data
new_scales_dis <- data.frame(new_scales_discriminant_data$construct_name[!new_scales_discriminant_data$construct_name %in% new_scales_convergent_data$construct_name]) #save returned values in dis
as.vector(new_scales_dis)
new_scales_only_discriminant_data <- setDT(copy_new_scales_discriminant_data, key = 'construct_name')[new_scales_dis] #select row based on dis (for discriminant_data) #only when the key is a factor/character variable

#only convergent data
new_scales_con <- data.frame(new_scales_convergent_data$construct_name[!new_scales_convergent_data$construct_name %in% new_scales_discriminant_data$construct_name]) #save returned values in con
as.vector(new_scales_con)
new_scales_only_convergent_data <- setDT(copy_new_scales_convergent_data, key = 'construct_name')[new_scales_con] #select row based on con (for convergent data) #only when the key is a factor/character variable
```

Number of constructs
```{r}
#only_discriminant: 4
nrow(new_scales_only_discriminant_data)
#only convergent: 15
nrow(new_scales_only_convergent_data)
#doublets: 64
nrow(new_scales_doublets_data)
```

#### Find doublets (with frequency): modified
```{r}
#as an index to discriminant_data$construct_name
#doublets
dou_split_modified <- data.frame(modified_scales_discriminant_data$construct_name[modified_scales_discriminant_data$construct_name %in% modified_scales_convergent_data$construct_name]) #save returned values in dou
as.vector(dou_split_modified)
copy_modified_scales_discriminant_data <- modified_scales_discriminant_data
copy_modified_scales_convergent_data <- modified_scales_convergent_data
modified_scales_d_freq1 <- setDT(copy_modified_scales_discriminant_data, key = 'construct_name')[dou_split_modified]
modified_scales_d_freq2 <- setDT(copy_modified_scales_convergent_data, key = 'construct_name')[dou_split_modified]
modified_scales_doublets_data <- data.frame(modified_scales_d_freq1[, 1], modified_scales_d_freq1[, 2] + modified_scales_d_freq2[, 2]) #make new data frame: take freq1 country_name and sum frq1 + freq1 construct_frequency
modified_scales_doublets_data

#only discriminant data
modified_scales_dis <- data.frame(modified_scales_discriminant_data$construct_name[!modified_scales_discriminant_data$construct_name %in% modified_scales_convergent_data$construct_name]) #save returned values in dis
as.vector(modified_scales_dis)
modified_scales_only_discriminant_data <- setDT(copy_modified_scales_discriminant_data, key = 'construct_name')[modified_scales_dis] #select row based on dis (for discriminant_data) #only when the key is a factor/character variable

#only convergent data
modified_scales_con <- data.frame(modified_scales_convergent_data$construct_name[!modified_scales_convergent_data$construct_name %in% modified_scales_discriminant_data$construct_name]) #save returned values in con
as.vector(modified_scales_con)
modified_scales_only_convergent_data <- setDT(copy_modified_scales_convergent_data, key = 'construct_name')[modified_scales_con] #select row based on con (for convergent data) #only when the key is a factor/character variable
```

Number of constructs
```{r}
#only_discriminant: 11
nrow(modified_scales_only_discriminant_data)
#only convergent: 6
nrow(modified_scales_only_convergent_data)
#doublets: 258
nrow(modified_scales_doublets_data)
```
